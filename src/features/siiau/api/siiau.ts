...
export type GlobalOfferResponse = {
  running: boolean;
  hasResult: boolean;
...
export async function fetchGlobalAcademicOffer(
  token: string,
): Promise<GlobalOfferResponse> {
  if (import.meta.env.DEV) {
    // eslint-disable-next-line no-console
    console.log('[OFFER][WEB] GET /offer/reload/status', {
      apiBaseUrl: API_BASE_URL,
      token: shortToken(token),
    });
  }
  ...
  let response: Response;
  try {
    response = await fetch(`${API_BASE_URL}/offer/reload/status`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    ...
    if (error instanceof DOMException && error.name === 'TimeoutError') {
      throw new Error('Tiempo de espera agotado al consultar oferta académica.');
    }
    throw new Error('No fue posible conectar con el servidor de oferta académica.');
  }
  ...
  if (!response.ok) {
    const message =
      typeof data.message === 'string'
        ? data.message
        : `No se pudo consultar oferta académica (${response.status})`;
    throw new Error(message);
  }

  if (import.meta.env.DEV) {
    // eslint-disable-next-line no-console
    console.log('[OFFER][WEB] reload/status OK', {
      httpStatus: response.status,
      running: data.running,
      hasResult: data.hasResult,
      totalCourses: Array.isArray(data.materias) ? data.materias.length : 0,
    });
  }

  return data as unknown as GlobalOfferResponse;
}
